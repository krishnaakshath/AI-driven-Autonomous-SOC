...
def get_valid_images():
    ...
    for path in image_paths:
        try:
            ...
            pass
    return valid_images[:2]

def create_ieee_pdf():
    pdf_filename = "FINAL_IEEE_Paper.pdf"
    print(f"Generating 5-6 Page Academic PDF: {pdf_filename}")
    ...
    state_vector_text = r"The state vector $S_t \in \mathbb{R}^{12}$ is defined as:<br/>1. Source Port Identity (Categorical)<br/>2. Destination Port Identity<br/>3. Packet Duration Timestamp ($t$)<br/>4. Bytes Sent ($T_x$)<br/>5. Bytes Received ($R_x$)<br/>6. AbuseIPDB Network Threat Signal ($1-100$)<br/>7. VirusTotal Hash Index Reputation<br/>8. Internal Isolation Forest Score ($[-1, 1]$)<br/>9. Protocol Type Encoded (TCP=1, UDP=2)<br/>10. Flag Rate Deviance<br/>11. Payload Regex Matched Count<br/>12. Time To Live (TTL) Delta"
    story.append(Paragraph(state_vector_text, normal))

    story.append(Paragraph("B. Action Space ($A_t$) and Reward Mapping ($R_t$)", heading2))
    story.append(Paragraph(r"The agent executes explicit deterministic actions $a_t \in \{0, 1, 2\}$, corresponding to \textit{Allow}, \textit{Rate-Limit}, and \textit{Block} algorithms. The subsequent reward formulation penalizes the agent heavily $\gamma (-100)$ for permitting anomalously verified events.", normal))
    ...
    story.append(Paragraph(r"$Q^*(s_t, a_t) = \mathbb{E} \left[ R_{t+1} + \gamma \max_{a'} Q^*(s_{t+1}, a') \mid s_t = s, a_t = a \right]$", math_style))
    story.append(Paragraph(r"Where $\gamma \in [0, 1]$ represents the discount factor dictating the network's emphasis on long-term defense sustainability rather than short-term local optima.", normal))
    story.append(Paragraph("By deploying a dual-network topology encompassing a static Target Network coupled with an active Policy Network, we mitigate catastrophic interference over long training cycles, culminating in a 100% convergence rate during testing phase. The Target Network is synced with the Policy Network weights only every $C$ epochs.", normal))
...
"""
    ...
    story.append(Paragraph("By implementing this block, CORTEX never crashes the primary Streamlit thread. Instead of relying on rigid string matching sequences like traditional SIEM queries, the flexible Regex allows the LLM to format the JSON naturally—whether indented, single-lined, or prefixed. This effectively grants deterministic properties to a non-deterministic generative brain model.", normal))
    ...
    story.append(Paragraph("To explicitly map the core topological components driving the physical application logic, the architecture utilizes uniquely encapsulated Python routing algorithms rendering distinct modular logic phases.", normal))
    ...
    modules = [
        ...
        ("Threat Intelligence Router", "Initiates asynchronous queries fetching realtime mapping from AbuseIPDB endpoints."),
        ("SOAR Workbench Visualizer", "Permits manual or autonomous formulation of active defense mitigation scripts."),
    ...
    print(f"Building condensed document...")
    try:
        doc.build(story)
        print("Success.")
