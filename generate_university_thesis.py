...
def create_university_thesis():
    ...
    def append_sanitized_code(directory_name, file_filter="*"):
        ...
        for filepath in sorted(glob.glob(os.path.join(target_dir, file_filter))):
            ...
            story.append(Paragraph(f"<b>Appendix: {directory_name}/{filename}</b>", sub_head))
            ...
            try:
                with open(filepath, 'r', encoding='utf-8') as f:
                    ...
                    lines = content.split('\n')
                    # Process in 80 line chunks to help reportlab paginate better
                    chunk_size = 80
    ...
    print("Building Document with Image integration... (Expected ~150-250 Pages)")
    try:
        doc.build(story)
